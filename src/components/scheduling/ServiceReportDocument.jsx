...
import { COMPANY, TREATMENT_METHODS } from "../../utils/constants";
...
function ServiceReportDocument({ appointment, client, technician, photos = [], signatureUrl = "" }) {
  if (!appointment || !client) return null;

  const materials = appointment.stockUsed || [];
  const methods = (appointment.treatmentMethods || [])
    .map((value) => TREATMENT_METHODS.find((method) => method.value === value)?.label)
    .filter(Boolean);
  const technicianLabel = [technician?.reference, technician?.name || technician?.username]
    .filter(Boolean)
    .join(" — ") || "Unassigned";
    ...
    <div className="service-form" id="service-form-print">
      ...
      <table className="sf-meta">
        <tbody>
          ...
          <Row label="Technician" value={technicianLabel} />
        </tbody>
      </table>
