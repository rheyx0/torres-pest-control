...
export async function createAccount(role, fields) {
  const table = TABLE_BY_ROLE[role];
  if (!table) {
    return { error: `The ${role} role has no table yet, so accounts cannot be created for it.` };
  }

  const columns = role === ROLES.ADMIN ? ADMIN_COLUMNS : STAFF_COLUMNS;
 ...
 */
export async function updateAccount(account, updatedFields) {
  const table = TABLE_BY_ROLE[account.role];
  if (!table) return { error: `Unknown role: ${account.role}` };

  if (updatedFields.role && updatedFields.role !== account.role) {
    return {
      error:
        "Changing a role is not supported yet — accounts are stored in a separate table per role. " +
        "Deactivate this account and create a new one with the correct role.",
    };
  }

  const payload = buildPayload(updatedFields);
  const { error } = await supabase.from(table).update(payload).eq("id", account.id);
  if (error) return { error: describeError(error) };

  return { updatedAt: payload.updated_at };
}
 ...
 */
export async function setAccountStatus(account, nextStatus) {
  const table = TABLE_BY_ROLE[account.role];
  if (!table) return { error: `Unknown role: ${account.role}` };
...
export async function setPassword(account, newPassword) {
  const table = TABLE_BY_ROLE[account.role];
  if (!table) return { error: `Unknown role: ${account.role}` };
